...
from resources.libs.kodiutils import addon_path, localize, log
...
class SpeedTest(Animation):
    ...
    def display_button_run(self, function='true'):
        if function == 'true':
            button_run_glowx = int((self.screenx / 2) - (300 / 2))
            button_run_glowy = int((self.screeny / 2) - (122 / 2) + 50)
    ...
    def display_button_close(self, function='true'):
        if function == 'true':
            self.button_close_glow = ControlImage(1520, 678, 300, 122, '', aspectRatio=0)
            ...
            self.button_close = ControlButton(1520, 678, 300, 122, localize(30951),
                                              focusTexture=self.image_button_run, noFocusTexture=self.image_button_run,
                                              alignment=2 | 4, textColor='0xFF000000', focusedColor='0xFF000000',
                                              shadowColor='0xFFCCCCCC', disabledColor='0xFF000000')
            ...
            self.button_close.setPosition(1520, 678)
    ...
    def display_ping_test(self, function='true'):
        if function == 'true':
            img_centertextx = int((self.screenx / 2) - (320 / 2))
            img_centertexty = int((self.screeny / 2) - (130 / 2) + 50)
            ...
            img_pingx = int((self.screenx / 2) - (600 / 2))
            img_pingy = int((self.screeny / 2) - (400 / 2))
            ...
            self.img_ping.setAnimations([(
                'conditional',
                'effect=fade start=0 end=100 delay=500 time=1000 condition=Control.IsVisible(%d)' % self.img_ping.getId()
            ), (
                'conditional',
    ...
    def display_gauge_test(self, function='true'):
        if function == 'true':
            ...
            img_gaugex = int((self.screenx / 2) - (548 / 2))
            img_gaugey = int((self.screeny / 2) - (400 / 2))
            
            img_gauge_arrowx = int((self.screenx / 2) - (66 / 2) - 5)
            img_gauge_arrowy = int((self.screeny / 2) - (260 / 2) - 60)
            ...
            dlul_prog_textboxx = int((self.screenx / 2) - (200 / 2))
            dlul_prog_textboxy = int((self.screeny / 2) - (50 / 2) + 170)
    ...
    def display_progress_bar(self, function='true'):
        if function == 'true':
            self.img_progress = ControlImage(660, 800, 600, 20, '', aspectRatio=0, colorDiffuse="0xFF00AACC")
            ...
            img_progressx = int((self.screenx / 2) - (200 / 2))
            img_progressy = int((self.screeny / 2) - (50 / 2) + 270)
            self.please_wait_textbox = ControlLabel(img_progressx, img_progressy, 200, 0, label='Please wait...',
    ...
    def display_results(self, function='true'):
        if function == 'true':
            self.img_results = ControlImage(1560, 40, 350, 139, '', aspectRatio=0)
            ...
            self.ping_textbox = ControlLabel(1583, 133, 75, 50, label='', textColor='0xFFFFFFFF')
            self.addControl(self.ping_textbox)

            self.dl_textbox = ControlLabel(1663, 133, 80, 50, label='', textColor='0xFFFFFFFF')
            self.addControl(self.dl_textbox)

            self.ul_textbox = ControlLabel(1781, 133, 80, 50, label='', textColor='0xFFFFFFFF')
    ...
    def show_end_result(self):
        self.img_final_results = ControlImage(1560, 40, 320, 144, '', aspectRatio=0)
    ...
    def show_end_result_sp(self):
        self.rec_speed = ControlTextBox(625, 605, 800, 300, textColor='0xFFFFFFFF')
    ...
    def config_gauge(self, speed, last_speed=0, time=1000):
        ...
        img_gauge_arrowx = (self.screenx / 2) - (66 / 2) + 28
        img_gauge_arrowy = (self.screeny / 2) + (260 / 2) - 88
        self.img_gauge_arrow.setAnimations([(
            'conditional',
